...
const ListarPacientes = () => 
{
  ...
  useEffect(() => 
  {
      const consultarPacientes = async() => {
         try 
         {
          const peticion = await fetch("https://630d6f2c109c16b9abe5ca2a.mockapi.io/pacientes")
      ...
      try 
        {
          ...
          if(confirmar)
          {
              const url = `https://630d6f2c109c16b9abe5ca2a.mockapi.io/pacientes/${id}`
              const peticion = await fetch(url,{
                  method:'DELETE',
              })
              const nuevosPacientes = pacientes.filter(word => word.id !== id)
